...
class HRAandMedicalAllowance(Document):
				...
				self.append("hra_and_medical_allowance_details",
								{
									...
									"hra": ((i.hra/date)*present_days)+retension_amt_hra ,
	...
	def before_save(self):
		...
		for i in self.get('hra_and_medical_allowance_details'):
			...
			if ssa:
				for j in ssa:
					frappe.db.set_value("Salary Structure Assignment", j.name, "medical_allowance", i.medical_allowance)
					frappe.db.set_value("Salary Structure Assignment", j.name, "hra", i.hra)
